...
\section{Sinais de Tempo Discreto e Transformada Discreta de \textit{Fourier}}
	...
	\subsection{Problema da Amostragem de Sinal Discreto}
		...
		\begin{equation}
			X_s(f) = \mathscr{F}[x(t)\delta_{T_s}(t)] = X(f)\ast\Delta_{T_s}(f) = X(f)\ast f_s\sum_{k=-\infty}^{\infty}\delta(f-kf_s) = \boxed{f_s\sum_{k=-\infty}^{\infty}X(f-kf_s)}
			\label{eq:3}			
		\end{equation}
		...
		No nosso caso concreto, teremos que utilizar esses conceitos para avaliar uma taxa de amostragem coerente bem como conceitos relativos ao tipo de método usado para representar digitalmente amostras de um sinal analógico.
